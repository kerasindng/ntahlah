...
from indomie.utils.FastTelethon import upload_file
...
@indomie_cmd(pattern="currency ([\\d\\.]+) ([a-zA-Z]+) ([a-zA-Z]+)")
...
@indomie_cmd(pattern="google ([\\s\\S]*)")
...
@indomie_cmd(pattern=r"lang (tr|tts) (.*)")
...
@indomie_cmd(pattern="yt(audio|video( \\d{0,4})?) (.*)")
...
@indomie_cmd(pattern="barcode(?: |$)([\\s\\S]*)")
async def bq(event):
    xx = await edit_or_reply(event, "`Processing...`")
    input_str = event.pattern_match.group(1)
...
@indomie_cmd(pattern=r"makeqr(?: |$)([\s\S]*)")
