...
from indomie import BLACKLIST_CHAT, BOTLOG_CHATID, CMD_HANDLER as cmd, CMD_HELP
from indomie.utils import edit_delete, indomie_cmd as memek
...
async def filter_incoming_handler(handler):
    ...
    try:
        if not (await handler.get_sender()).bot:
            try:
                from indomie.modules.sql_helper.filter_sql import get_filters
...
async def add_new_filter(new_handler):
    ...
    try:
        from indomie.modules.sql_helper.filter_sql import add_filter
    except AttributeError:
...
async def remove_a_filter(r_handler):
    ...
    try:
        from indomie.modules.sql_helper.filter_sql import remove_filter
    except AttributeError:
