...
from indomie.utils import bash, indomie_cmd, indomie_handler
        ...
        try:
            if pic.endswith((".tgs", ".webp")):
                ...
                shites = await event.client.send_message(
                    event.chat_id,
                    f"❏ **{owner} Kembali Onlen Untuk Menjadi Sasimo**\n└ **Dari AFK** `{total_afk_time}` **Yang Lalu**",
                )
            else:
                shite = await event.client.send_message(
                    event.chat_id,
                    f"❏ **{owner} Pengangguran Sok Sibuk Baru Balik!**\n└ **Dari AFK** `{total_afk_time}` **Yang Lalu**",
                    file=pic,
                )
        except BaseException:
            shite = await event.client.send_message(
                event.chat_id,
                f"❏ **{owner} Kembali Onlen**\n└ **Dari AFK** `{total_afk_time}` **Yang Lalu**",
            )
