...
async def warn(e):
    ...
    if count + 1 >= number:
        if "ban" in action:
            try:
                await pepek.edit_permissions(e.chat_id, user, view_messages=False)
            except BaseException:
                return await e.eor("`Something Went Wrong.`", time=5)
        elif "kick" in action:
            try:
                await pepek.kick_participant(e.chat_id, user)
            except BaseException:
                return await e.eor("`Ada yang salah.`", time=5)
        elif "mute" in action:
            try:
                await pepek.edit_permissions(
...
async def warnset(e):
    ...
    if "|" in ok:
        ...
        memek.set_key("SETWARN", f"{number} {action}")
