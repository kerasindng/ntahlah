...
@indomie_cmd(pattern="notes$")
async def notes_active(svd):
    try:
        from indomie.modules.sql_helper.notes_sql import get_notes
...
@indomie_cmd(pattern="clear (\w*)")
async def remove_notes(clr):
    """For .clear command, clear note with the given name."""
    try:
        from indomie.modules.sql_helper.notes_sql import rm_note
    except AttributeError:
...
@indomie_cmd(pattern="save (\w*)")
async def add_note(fltr):
    try:
        from indomie.modules.sql_helper.notes_sql import add_note
    except AttributeError:
...
async def incom_note(getnt):
    ...
    try:
        if not (await getnt.get_sender()).getnt.client:
            try:
                from indomie.modules.sql_helper.notes_sql import get_note
            except AttributeError:
                return
...
@indomie_cmd(pattern="rmbotnotes (.*)")
