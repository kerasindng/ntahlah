...
@indomie_cmd(pattern="eval(?:\s|$)([\s\S]*)")
async def _(event):
    expression = event.pattern_match.group(1)
    if not expression:
        return await event.edit("**Berikan Code untuk di eksekusi.**")
    if expression in ("indomie.session", "config.env"):
...
@indomie_cmd(pattern="exec(?: |$|\n)([\s\S]*)")
async def run(event):
    code = event.pattern_match.group(1)
    if not code:
        return await event.edit("**Read** `.help exec` **for an example.**")
    if code in ("indomie.session", "config.env"):
        return await event.edit("`Itu operasi yang berbahaya! Tidak diperbolehkan!`")
