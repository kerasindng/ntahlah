...
class MongoDB:
    def __init__(self, key, dbname="IndomieDB"):
    ...
    def flushall(self):
        self.dB.drop_database("IndomieDB")
        self._cache = {}
        return True
...
class SqlDB:
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS Indomie (indomieCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('Indomie')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'indomie'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM indomie")
    ...
    def set_key(self, key, value):
        try:
            self._cursor.execute(
                f"ALTER TABLE Indomie DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE Indomie ADD {key} TEXT")
        self._cursor.execute(
    ...
    def del_key(self, key):
        if key in self._cache:
            del self._cache[key]
        try:
            self._cursor.execute(f"ALTER TABLE Indomie DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE indomie")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS Indomie (indomieCli varchar(70))"
        )
        return True
