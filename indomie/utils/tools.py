...
def post_to_telegraph(title, html_format_content):
    ...
    auth_name = "Userbothon"
    auth_url = "https://github.com/Friscay/userbothon"
...
async def animator(media, mainevent, textevent):
    ...
    memek = await mainevent.client.download_media(media, TEMP_DOWNLOAD_DIRECTORY)
    await textevent.edit("__🎞Converting into Animated sticker..__")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {memek} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an animate.webm"
    )  # pain
    os.remove(memek)
